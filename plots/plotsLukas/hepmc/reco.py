...
#            plot.histos[-3][0].Add(hep_histo[0])
        plot.histos[4][0].Add(plot.histos[1][0]) #HG
        plot.histos[4][0].Add(plot.histos[2][0]) #GH
        plot.histos[4][0].Add(plot.histos[3][0]) #HH
    ...
    for plot in plots:
        for i_h, h in enumerate(plot.histos):
            for j_hi, hi in enumerate(h):
                if i_h == len(plot.histos)-1:
                    # fill style for bg
                    hi.style = styles.fillStyle(bgcolors[j_hi])
                elif i_h == len(plot.histos)-3:
                    # fill style for signal PP
                    hi.style = styles.lineStyle( ROOT.kRed, width=3  )
                elif i_h == len(plot.histos)-2:
                    # fill style for signal PP
    ...
    for log in [False, True]:
        plot_directory_ = os.path.join( plot_directory, 'hepmc/reco_%s'%args.version, sample_directory, args.selection, args.pdf, "c%s"%str(args.c).replace(".","p"),"log" if log else "lin" )
...
SMSample.weight = lambda event, sample: nloXSec/hepSample.samples_dict['PP'].xSection
